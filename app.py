...
st.set_page_config(
    page_title="Panorama Salarial na Área de Dados",
    page_icon="📊",
    layout="wide",
)
...
# --- Conteúdo Principal ---
st.title("🎲 Panorama Salarial na Área de Dados")
...
with col_graf1:
    if not df_filtrado.empty:
        ...
        grafico_cargos = px.bar(
            ...
            y='cargo',
            orientation='h',
            title="Ranking dos cargos por salário médio",
...
with col_graf2:
    if not df_filtrado.empty:
        grafico_hist = px.histogram(
            ...
            x='usd',
            nbins=30,
            title="Visão geral dos salários anuais",
...
if not df_filtrado.empty:
    # Foco nos cargos principais (opcional, mas recomendado)
    cargos_foco = ['Data Scientist',
                   'Data Analyst', 'Machine Learning Engineer']
    df_tempo = df_filtrado[df_filtrado['cargo'].isin(cargos_foco)]

    salario_tempo = (
        df_tempo
        .groupby(['ano', 'cargo'])['usd']
        .mean()
        .reset_index()
    )
    ...
    grafico_evolucao = px.line(
        ...
        color='cargo',
        markers=True,
        title='Evolução do salário médio por cargo ao longo do tempo',
        labels={
            'ano': 'Ano',
